...
<%-- tpl:put name="headarea" --%>
	<title>Lot Reclassification</title>
<%-- /tpl:put --%>
...
Vector dualDD = InqInventory.getListDualDropDownFacilityWarehouse(ii.getFacilityFilter());
 %>
<form  name = "inqInventory" action="/web/CtlInventoryNew?requestType=listLotReclass" method="post">
<input type="hidden" name="environment" value="<%=ii.getEnvironment() %>" />

<h1>Reclassify Lots to Another Status</h1>
...
<fieldset>
    <legend>Selection Criteria for Choosing Lots to Reclassify</legend>
	...
	<table class="styled full-width">
		...
		<tr>    
			<td>
			    <label>Choose Type of Reclassification:</label>
			</td>
			<td>
			    <%= InqInventory.buildDropDownStatus(ii.inqStatus) %>
			</td>
		</tr>
...
%>		
		<tr>
			...
			<td>
			<%= HTMLHelpersInput.inputBoxText("inqRemark", ii.getInqRemark(), "Remark", 20, 20, "N", "N") %>
			</td>
		</tr>
<%
  }
