...
<%  InqOperations io = (InqOperations) request.getAttribute("inqOperations");
    ...
    ManufacturingFinance labor = (ManufacturingFinance) io.getBean().getWarehousing().get("Labor");
    if (labor == null) {
        labor = new ManufacturingFinance();
    }
    ManufacturingFinance benefits = (ManufacturingFinance) io.getBean().getWarehousing().get("Benefits");
    if (benefits == null) {
        benefits = new ManufacturingFinance();
    }
    ManufacturingFinance supplies = (ManufacturingFinance) io.getBean().getWarehousing().get("Supplies"); 
    if (supplies == null) {
    ...
    <% BigDecimal plantTotal = (BigDecimal) request.getAttribute("plantTotal");
       ...
       request.setAttribute("plantTotal", plantTotal);
        ...
        <%  KeyValue keys = io.getCommentKeys();
            keys.setKey1("warehousing");
            request.setAttribute("keys",keys); %>


         <%  if (io.getRequestType().equals("monthly")) { %>
            <% request.setAttribute("weeklyCommentType","warehousing"); %>
