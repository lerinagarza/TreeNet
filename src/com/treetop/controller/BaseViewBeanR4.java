...
public abstract class BaseViewBeanR4 {
	...
	public void populate(HttpServletRequest request) {
		...
		for (Field field: fields) {
			...
			if (field.getType() == String[].class) {
				...
				if (paramValues != null) {
					try {
						Class[] paramClsType = {String[].class};
						Method method = getMethod(cls, fieldName, paramClsType, MethodType.SET);
	...
	/**
	 * Uses annotation @UrlPathMapping to map fields in the view bean to url path parts
	 *       @UrlPathMapping({"requestType","itemNumber"})
	 * 
	 * 		 https://treenet.treetop.com/web/CtlApp/details/102166 
	 * 
	 * 		 will load field "requestType" with value "details" 
	 * 			   and field "itemNumber"  with value "102166"
	 * 
	 * 		assumes the servlet CtlApp was mapped as "/CtlApp/*"
	 * 
	 * @param request
	 */
	private void populatePaths(HttpServletRequest request) {
		...
		try {
			...
			if (pathMapping != null) {
				...
				Hashtable<String, String> ht = MappingUtility.getMappingValues(request, pathMapping);
				
				for (String key : ht.keySet()) {
					try {
						Method method = getMethod(this.getClass(), key, MethodType.SET);
						method.invoke(this, ht.get(key));
					} catch (Exception e) {}
				}
				
				
				
			}	//end if pathMapping is null

			
		} catch (Exception e) {
			System.err.println("Exception @ BaseViewBeanR4.populatePaths().  " + e);
		}
	...
	public String buildParameterResendInputs() {
		return buildParameterResendInputs(new String[] {});
	}
	 ...
	 * @return
	 */
	public String buildParameterResendInputs(String[] excludeFields) {
		StringBuffer resend = new StringBuffer();
		try {
			Class c = this.getClass();
			List<Field> fields = getAllFields(new LinkedList<Field>(), c);
